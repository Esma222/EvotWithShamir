...
contract ShamirVoting {
    ...
    function random() internal view returns (uint256) {
        return uint(keccak256(abi.encodePacked(block.timestamp))) % 1000;
    }
   
    // Polinomun katsayılarını, sırrı ve sır paylarını oluştur
    function generateSecretAndShares(address[] memory voterAddresses) private {
        secret = random();
        // Polinomun katsayılarını rastgele oluştur
    ...
    // Oy kullanma
    function vote(uint256 candidateIndex, uint256 _x, uint256 _y) public onlyVoter {
